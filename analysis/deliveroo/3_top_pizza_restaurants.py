...
def get_top_pizza_restaurants_deliveroo(db_path, limit=10):
    """
    Connects to the deliveroo.db database and retrieves the top N pizza restaurants
    based on ratings.
    ...
    Returns:
        list: A list of tuples, where each tuple contains the restaurant's
              name, rating, and number of ratings.
    """
    ...
    query = """
        SELECT
            ...
            r.rating,
            r.rating_number AS review_count
        FROM
            restaurants r
        WHERE
            r.id IN (
...
if __name__ == '__main__':
    ...
    top_10_pizza = get_top_pizza_restaurants_deliveroo(db_path)

    print("Top 10 Pizza Restaurants (by Rating - Deliveroo):")
