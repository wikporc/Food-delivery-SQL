...
OUTPUT_MAP_PATH = 'analysis/takeaway/delivery_dead_zones_map.png'

def generate_dead_zones_map_takeaway(output_dir='analysis/takeaway'):
    """
    Generates a choropleth map highlighting delivery "dead zones" (areas with low/no restaurant coverage)
    for the Takeaway platform based on postal codes.
    """
    print("Generating Delivery Dead Zones Map for Takeaway...")
    ...
    ax.set_title('Takeaway: Restaurant Coverage by Postal Code (Delivery Dead Zones)', fontdict={'fontsize': '18', 'fontweight': '3'})
    ax.set_axis_off()
    
    # Add a separate legend entry for dead zones if they exist
    if not dead_zones.empty:
        handles, labels = ax.get_legend_handles_labels()
        # Find the dead zone label and move it to the end or just ensure it's displayed
        # This part might need manual adjustment depending on matplotlib version
        # For simplicity, we just ensure the label exists.
        pass
...
if __name__ == '__main__':
    generate_dead_zones_map_takeaway()
