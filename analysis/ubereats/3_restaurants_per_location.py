...
# Assuming zipcode-belgium.json is in the parent 'analysis' directory
...
if os.path.exists(POSTAL_CODE_MAP_FILE):
    ...
    # print(f"Loaded {len(postal_code_to_city)} postal code mappings from {POSTAL_CODE_MAP_FILE}.")
else:
...
def analyze_restaurants_per_location_ubereats(output_dir='analysis/ubereats'):
    """
    Analyzes how restaurants are distributed across different cities on the Uber Eats platform.
    ...
    df_restaurants['standardized_city'] = df_restaurants['postalCode'].apply(get_standardized_city)
    ...
    df_city = df_restaurants.groupby('standardized_city').agg(
        restaurant_count=('primarySlug', 'nunique') # Use primarySlug to count unique restaurants
    ).reset_index()
    ...
    plt.ylabel('City')
    plt.title('Top 20 Cities by Number of Unique Restaurants (Uber Eats - Standardized)')
    ...
    plt.title('Restaurant Concentration in Top 10 Cities (Uber Eats - Standardized)')
    plt.axis('equal')
    plt.savefig(os.path.join(output_dir, 'restaurants_concentration.png'))
    plt.close()

    print("\nTop 10 cities with most unique restaurants (Uber Eats - Standardized):")
...
if __name__ == '__main__':
    # db_path is no longer needed directly here, as we load from CSV
    analyze_restaurants_per_location_ubereats()