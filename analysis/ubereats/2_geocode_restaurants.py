...
OUTPUT_CSV = 'analysis/ubereats/restaurant_postal_codes.csv' # Output file
...
def geocode_restaurants_with_checkpointing_ubereats(db_path):
    conn = sqlite3.connect(db_path)
    # Use 'id' for primarySlug, 'location__longitude', 'location__latitude' for coordinates
    restaurants_db_df = pd.read_sql_query("SELECT id AS primarySlug, location__longitude AS longitude, location__latitude AS latitude FROM restaurants", conn)
    conn.close()
...
if __name__ == '__main__':
    ...
    geocode_restaurants_with_checkpointing_ubereats(db_path)